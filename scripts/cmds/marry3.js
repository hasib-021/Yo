...
const localBgPath = path.join(__dirname, "cache", "marry3_bg.jpg");


const avatarConfig = {
  sender: { x: 250, y: 1, size: 90 },
  mention: { x: 350, y: 70, size: 90 }
};

module.exports = {
  config: {
    name: "marry3",
    aliases: ["married3","biya3"],
    version: "1.0",
    author: "Hasib",
    countDown: 5,
    role: 0,
    description: "💍 Create a romantic marriage image on a new background!",
    category: "love",
    guide: {
  ...
  onStart: async function ({ event, message, usersData, getLang }) {
    ...
    try {
      ...
      const imgPath = path.join(tmpDir, `${uid1}_${uid2}_marry3.jpg`);
      ...
      console.error("❌ Error in marry3.js:", err);
